...
namespace SDKTemplate
{
    ...
    public sealed partial class SecondaryViewPage : Page
    {
        ...
        public ObservableCollection<Item> itemsLv1 = new ObservableCollection<Item>() { new Item() { Field1 = "AA" } };
        public List<string> listacheia = new List<string>();

        public SecondaryViewPage()
        {
            this.InitializeComponent();
            TitleBox.LosingFocus += SecondTextBox_LosingFocus;
            TitleBox.LostFocus += SecondTextBox_LostFocus;

            for (int i = 0; i < 50; i++)
            {
                itemsLv1.Add(new Item()
                {
                    Field1 = "Campo 1" + i.ToString(),
                    Field2 = "Campo 2" + i.ToString(),
                    Field3 = "Campo 3" + i.ToString(),
                    Field4 = "Campo 4" + i.ToString(),
                    Field5 = "Campo 5" + i.ToString(),
                    Field6 = "Campo 6" + i.ToString(),
                    Field7 = "Campo 7" + i.ToString(),
                    Field8 = "Campo 8" + i.ToString(),
                    Field9 = "Campo 9" + i.ToString(),
                });
            }
            lv1.ItemsSource = lv2.ItemsSource = lv3.ItemsSource = lv4.ItemsSource = lv5.ItemsSource = lv6.ItemsSource = itemsLv1;
            Task.Factory.StartNew(() =>
            {
                ...
                while (true)
                {
                    int amountOfRows = rdm.Next(1, 50);
                    for (int i = 0; i < amountOfRows; i++)
                    {
                        int rowToReplace = rdm.Next(0, 50);
                        int rowToCopy = rdm.Next(0, 50);

                        var tempReplacingRow = itemsLv1[rowToReplace];

                        Dispatcher.RunAsync(Windows.UI.Core.CoreDispatcherPriority.Normal, () =>
                        {
                            itemsLv1[rowToReplace] = itemsLv1[rowToCopy];
                            itemsLv1[rowToCopy] = tempReplacingRow;
                        });
                    }
                    //Task.Run(() => { listacheia.Add(File.OpenText("Arquivo").ReadToEnd()); } /*listacheia.Add(File.OpenText("Arquivo").ReadToEnd())*/);
                    Task.Delay(50).Wait();
                }
            });
